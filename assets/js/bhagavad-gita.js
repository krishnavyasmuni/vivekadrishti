(() => {
  ...
  const url = (n) => 'https://raw.githubusercontent.com/vedicscriptures/bhagavad-gita/main/slok/bhagavadgita_chapter_' + chapter + '_slok_' + n + '.json';
  const makeVerse = (d) => {
    const n = d.verse;
    ...
    return '<article class="gita-verse" id="gita-' + chapter + '-' + n + '">' +
      ...
      '<p class="gita-translation"><strong>English translation.</strong> ' + english + '</p>' +
      ...
      '<details class="gita-details"><summary>Śrīdhara transliteration</summary><div class="gita-reveal"><p><em>' + lines(d.srid && d.srid.sc ? d.srid.sc : 'Not applicable.') + '</em></p></div></details>' +
  ...
  const load = async () => {
    ...
    const data = await Promise.all(Array.from({length: counts[chapter - 1]}, (_, i) => fetch(url(i + 1)).then((r) => r.json())));
